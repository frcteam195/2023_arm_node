...
TRANSITIONS = [
    ArmStateMachine.States.HIGH_CONE_FRONT,
    ArmStateMachine.States.HIGH_CONE_BACK,
]

class IntermediateHighConeState(StateMachine.State):
    ...
    def get_enum(self):
        if self.side is ArmStateMachine.GoalSides.FRONT:
            return ArmStateMachine.States.INTERMEDIATE_HIGH_CONE_FRONT
        else:
    ...
    def transition(self) -> Enum:
        if self.arm.is_at_setpoint_raw(0.03, 0.03) and \
