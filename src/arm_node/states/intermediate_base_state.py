...
class IntermediateBaseState(StateMachine.State):
    ...
    def transition(self) -> Enum:
        if self.machine.goal_state == ArmStateMachine.States.FORCE_HOME:
            self.machine.set_goal(ArmStateMachine.States.HOME)
            return ArmStateMachine.States.HOME
        elif self.arm.is_at_setpoint_raw(0.2, 0.25) and self.arm.is_retracted() and self.is_base_arm_opposite() and self.arm.wrist_at_setpoint(0.04):
            if self.side is not ArmStateMachine.get_goal_side(self.machine.goal_state):
                return ArmStateMachine.States.HOME
            ...
            return self.machine.goal_state
             ...
             numpy.sign(self.arm.upperMotor.get_sensor_position()) == numpy.sign(self.arm.upperMotor.get_setpoint()):
